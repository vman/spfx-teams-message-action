...
export default class TeamsMessageActionWebPart extends BaseClientSideWebPart<ITeamsMessageActionWebPartProps> {
  ...
  public render(): void {
    ...
    const queryStringParams = new UrlQueryParameterCollection(window.location.href);
    if (!messageId) {
      messageId = queryStringParams.getValue("augMessageId");
    }

    const replyToId = queryStringParams.getValue("augReplyToId");
